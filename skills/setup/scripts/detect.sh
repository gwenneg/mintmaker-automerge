...
[ -n "$pyproj" ] && row "Python, pyproject" "$(printf '%s' "$pyproj" | ticks)" "$LIB"
eco cargo '(^|/)Cargo\.toml$' "Cargo" "$LIB"
eco bundler '(^|/)Gemfile$' "Bundler" "$LIB"
eco dockerfile "$CONTAINERFILES" "Container images" "🛑 runtime under every test, stays manual"
eco pre-commit '(^|/)\.pre-commit-config\.yaml$' "pre-commit" "$NORULE"
eco github-actions "$ACTIONFILES" "GitHub Actions" "🟢 allow-list of actions"
...
# One line per FROM, minus stage names, scratch and variables; the image is the first word after the flags.
...
if [ -f .github/dependabot.yml ]; then
  ...
  ' .github/dependabot.yml | while read -r eco dir; do
    d="${dir#/}"; [ -z "$d" ] && d=.
    case "$d" in
...
if [ -n "$BASES" ]; then
  ...
  say "$BASES" | sed -E 's/^([^:]+): (.*) \((.*)\)$/\2/' | sort | uniq -c | sort -rn | while read -r cnt img; do
    case "$img" in
