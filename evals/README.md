...
`(Recommended)` option of every menu, answers `Stop here` at Step 12 so
nothing is committed or pushed, then checks:

- the welcome screen comes first, the twelve step headers come in order, and
...
| Fixture | Exercises |
|---|---|
| `java` | Maven with Quarkus (manual-review candidate), Maven wrapper, tag-only base images, tag-pinned actions, no Renovate config, a second Konflux component on a `security-compliance` branch switched off through the Step 2 how-to, Dependabot overlap removed in Step 9 |
...
forever, the very thing the skill warns about in Step 10.
